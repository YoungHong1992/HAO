...
- **它自己有没有记着对外地址？** 公开地址 / 站点 URL、允许来源（Origin / Referer
  ...
  这些都不在 Nginx 里，换域名或换协议时要跟着改；漏改的现象是**首页正常、
  一登录/一提交就报错**。理由与做法见 `references/site.md` 的「来源校验教训」。
- **它启动时有没有会拒绝启动的自检门？** 有的应用把「对外协议必须是 HTTPS」
  「必须配验证码 / 签名密钥」写成启动校验，不满足就直接拒绝启动 —— 给这种应用加
  80→443 跳转的结果是**服务起不来**，比明文更糟。先读懂它的启动校验再定协议。
...
- 站点：**真的取一次内容**（`curl -sS -o /dev/null -w '%{http_code}' -H "Host: $DOMAIN" http://127.0.0.1/`），
  不是 2xx/3xx 就停下
- **反代的是应用时，再发一次非 GET 请求**（一次注定失败的登录就够）：GET 通常不做
  来源校验，**首页 200 证明不了可用** —— 来源校验 / CSRF / 中间件这类故障只在 POST
  上出现，现象是"首页好好的、用户一操作就报错"。回来的不是来源 / CSRF 类的 403，
  就说明这一层过了
- 探测结果**隔一两秒复测一次再下结论**：`systemctl reload nginx` 是优雅切换，
  紧接着的第一次请求可能仍由旧 worker 按改动前的配置应答（真实踩过：改完 vhost
  后第一次 `curl` 拿到的是改动前的 200，复测才得到正确的 301）。拿第一次的结果
  报成功或失败都会报错
