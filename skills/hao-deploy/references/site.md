...
5. **反代应用时，已按上游文档配置公开 HTTPS URL、TLS 终止和可信代理，并验证
   实际交互流程**（见下面「来源校验教训」）。内部 HTTP 监听不妨碍 Nginx 对外提供 HTTPS。

前四条有未满足项时，不启用跳转；只有应用支持该访问方式，且符合用户确认的部署
方案时，才填 `include /etc/nginx/snippets/$CONF_NAME.conf;`（80 直接提供服务）。
第 5 条不满足时，先查清兼容性或验证失败的原因，再与用户确定方案，不自动回退到明文。
两种填法都是**一行**，别把跳转那段 location 直接写进模板 —— 占位符的值必须是单行，
理由见模板头部。
...
反代一个应用（面板、网关、自建服务）并给它换对外地址时，应用可能仍使用旧的
公开地址或可信来源配置。例如，从 `http://<IP>` 改为 `https://<域名>` 后，
首页可能仍返回 200，但登录或提交被来源校验拒绝。具体校验规则以应用文档为准。

要查的东西（各应用叫法不同，按语义找）：公开地址 / 站点 URL、允许来源
（Origin / Referer 白名单）、CORS、OAuth 回调地址、Cookie 的 `Secure` 位、
CSRF 的信任来源、TrustedHost / AllowedHosts。具体配置键和环境变量名以目标应用的
官方文档为准，不假定所有应用使用相同名称。

还要区分**应用内部监听协议**与**用户访问的公开协议**：Nginx 可以终止 TLS，
再通过回环 HTTP 连接应用。按上游文档配置公开 HTTPS URL、转发头（如
`X-Forwarded-Proto`）和可信代理范围，让应用正确识别原始协议、主机名及端口；
不能仅因内部监听 HTTP 就判断应用不支持公开 HTTPS，也不能只加跳转而忽略应用配置。

有些应用还会在启动时校验公开协议或必需密钥，不满足就拒绝启动。处理顺序是：

1. 只读检查上游文档、配置模板、相关非敏感配置和启动日志，确认启动校验与反代要求；
2. 向用户说明改地址涉及的配置和兼容性限制，确认方案后再修改；
3. 确认服务正常启动，再通过公开 URL 验证首页及应用实际提供的交互流程。

交互验证应按应用文档选用无破坏性的测试流程，明确预期结果。对提供 POST 等非 GET
接口的应用，模拟浏览器请求：Origin / Referer 应对应公开 URL，并按要求先获取会话
Cookie 和 CSRF token，再调用目标接口。使用测试账号或用户确认的测试数据，避免
反复失败登录触发锁定，也不要让测试提交修改真实业务数据。

结合预期响应和必要的应用日志判断是否通过。**没有来源 / CSRF 类 403 不等于验证成功**：
重定向可能只到了登录页，404 / 500 也不能证明请求已通过相关校验；缺少合法 CSRF token
导致的拒绝则可能是正常保护。一次预期失败的登录最多只能验证该请求经过的路径，不能
代替成功登录或提交的验证。无法完成的流程应在交接中标为未验证，不要汇报为全部可用。

如果上游明确不支持所需的公开 HTTPS 部署方式，先说明限制及可选方案，让用户决定后
再继续；不自动改为明文部署，也不通过关闭来源校验或 CSRF 保护来换取测试通过。
