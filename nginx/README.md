...
通俗地说，**Nginx 是服务器入口处的“接线员兼门卫”**。

- **接线员**：根据域名、端口或路径，判断请求要找哪个服务，再把请求转接过去。
- **门卫**：统一处理 HTTPS 证书、访问日志、连接限制和部分安全规则。
- **统一入口**：外部通常只需要访问标准的 80/443 端口，内部服务可以继续使用各自端口。

例如，一台服务器只有一个公网 IP，但运行了两个服务：

```text
api.example.com -> Nginx -> New-API:3000
cpa.example.com -> Nginx -> CliproxyAPI:8317
```

这里两个域名类似两个“分机号”，Nginx 根据分机号完成转接。
...
| 场景 | 访问方式 | 是否需要 Nginx |
|---|---|---|
| 单个服务直接访问 | `IP:服务端口` | 技术上可以不需要 |
| 多个服务直接访问 | 每个服务使用不同端口 | 技术上可以不需要 |
| 多个服务共用 80/443 | 使用不同域名交给 Nginx 区分 | 需要 |
| 希望统一处理 HTTPS、日志和 WebSocket | 请求先到 Nginx | 建议需要 |

同一个 IP 上的多个服务不能同时直接监听相同的 80/443 端口。如果没有域名，通常只能
通过 `IP:不同端口` 访问。当前 HAO 即使选择 `ip` 或 `http` 访问模式，New-API 和
CliproxyAPI 仍使用 Nginx 作为统一入口；直接暴露内部端口不属于当前正式部署流程。
...
| 项目 | 最低配置 | 推荐配置 |
|------|---------|---------|
| **操作系统** | Debian 13/12，Ubuntu 26.04/24.04/22.04 LTS | 当前支持矩阵内版本 |
| **内存** | 256MB | 512MB+ |
| **磁盘** | 100MB 可用空间 | 500MB+ |
| **内核版本** | 4.9+（BBR 支持） | 5.4+ |
| **权限** | root | root |
